...
let trade_in_pool (param : trade_in_pool) (storage : storage) : result = 
    ...
    let rate_out = 
        ...
        | Some rate -> rate in 
    // some calculations 
    let l = sqrt (storage.outstanding_pool_tokens / (rate_in * rate_out)) in 
    let qty_out = l * rate_in - storage.outstanding_pool_tokens / (l * rate_out + qty) in 
    // burn 
    let op_transfer_in = 
        match (Tezos.get_entrypoint_opt "%transfer" Tezos.self_address : transfer contract option) with 
        | None -> (failwith error_FA2_SENDER_HOOK_FAILED : operation)
    ...
    let op_transfer_out = 
        ...
        | Some entrypt -> (
            let txn_data = 
                let transfer_to = { to_ = Tezos.self_address ; token_id = token_out ; amount = qty_out ; } in 
                    { from_ = Tezos.sender ; txs = [ transfer_to ; ] ; } in 
            Tezos.transaction txn_data 0tez entrypt ) in 
    // update exchange rate of token_in 
    let new_rate_in = (rate_in * )
    let storage = { storage with 
        exchange_rates = Big_map.update token_in  }
